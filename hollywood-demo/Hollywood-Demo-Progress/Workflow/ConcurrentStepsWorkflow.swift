...
struct ConcurrentStepsWorkflow: CompositeWorkflowAction {
    ...
    func execute() async throws -> String {
        ...
        async let resultA = execute(GenerateNumericString(iterations: 10, delay: .milliseconds(200), pendingUnitCount: 25))

        // This action produces 15% of the workflow's progress.
        async let resultB = execute(GenerateNumericString(iterations: 100, delay: .milliseconds(50), pendingUnitCount: 15))

        // This action produces 20% of the workflow's progress.
        async let resultC = execute(GenerateNumericString(iterations: 4, delay: .seconds(1), pendingUnitCount: 20))

        // This action produces 40% of the workflow's progress.
        async let resultD = execute(GenerateNumericString(iterations: 500, delay: .milliseconds(2), pendingUnitCount: 40))

        return try await resultA + resultB + resultC + resultD
    }
}
